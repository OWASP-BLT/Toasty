...
# Installation state mapping - lazily import to avoid circular imports
def get_installation_state_mapping():
    from toasty.aibot.db_models import InstallationState
...
def get_repo_state_updates():
    from toasty.aibot.db_models import RepoState
...
# These are computed lazily at first access
INSTALLATION_STATE_MAPPING = None
REPO_STATE_UPDATES = None


def _init_state_mappings():
    global INSTALLATION_STATE_MAPPING, REPO_STATE_UPDATES
    if INSTALLATION_STATE_MAPPING is None:
        INSTALLATION_STATE_MAPPING = get_installation_state_mapping()
    if REPO_STATE_UPDATES is None:
